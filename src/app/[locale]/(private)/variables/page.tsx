  ...
  return (
    <div className="p-6 space-y-3 max-w-4xl mx-auto">
      ...
      <p className="text-muted-foreground">
        {t('subtitlePart1')} {t('subtitlePart2')}{' '}
      ...
      {variables.length === 0 ? (
        ...
        <div className="space-y-2 border rounded-md p-4">
          <h3 className="mb-4">
            {t('yourVariables')} ({variables.length})
          </h3>
          {variables.map((v, i) => (
      ...
      <div className="flex items-start space-x-2 text-gray-600 text-sm border rounded-md p-4">
        ...
        <p>
          {t('howToUsePart1')} <code>{'{{variableName}}'}</code>{' '}
          {t('howToUsePart2')} {t('howToUsePart3')}{' '}
          <code>{'{{API_BASE_URL}}/users'}</code> {t('howToUsePart4')}
        </p>
      </div>
    </div>
  );
